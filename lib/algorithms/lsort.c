...
void SORTCALL merge_subroutine(int64_array *a, uint64_t lo, uint64_t pivot,
                               uint64_t hi) {
...
int64_t SORTCALL hoare_partition_around(int64_t *arr, int64_t lo, int64_t hi,
                                        int64_t x) {
  int64_t i = lo - 1;
  int64_t j = hi + 1;
  int64_t tmp;
...
void SORTCALL modified_quicksort(int64_t *a, int64_t lo, int64_t hi,
                                 int64_t k) {
  while (lo < hi - k + 1) {
...
int64_t SORTCALL quick_select(int64_t *a, int64_t lo, int64_t hi, uint64_t i) {
  ...
  if (i <= k) {
    ...
    return quick_select(a, q + 1, hi, i - k);
}

int64_t SORTCALL quick_select_iterative(int64_t *a, int64_t lo, int64_t hi,
                                        uint64_t i) {
  while (lo < hi) {
    int64_t q = hoare_partition(a, lo, hi);
    uint64_t k = q - lo + 1;
...
int64_t SORTCALL linear_select(int64_t *a, int64_t lo, int64_t hi, uint64_t i) {
  ...
  if (i <= k) {
    ...
    return linear_select(a, q + 1, hi, i - k);
}
...
int64_sub max_crossing_subarray(int64_t *a, int64_t lo, int64_t mid,
                                int64_t hi) {
  int64_sub s = {0};
  int64_t i;
...
SORTAPI int64_sub SORTCALL recursive_subarray(int64_t *a, int64_t lo,
                                              int64_t hi) {
  if (lo == hi) {
...
strassen_matrix *subtract_matrix(strassen_matrix *A, strassen_matrix *B) {
  ...
  for (uint64_t i = 0; i < n; i++) {
    for (uint64_t j = 0; j < n; j++) {
      S->ptr[i][j] =
          A->ptr[A->rmin + i][A->cmin + j] - B->ptr[B->rmin + i][B->cmin + j];
    }
  }
  return S;
}
...
strassen_matrix *SORTCALL strassen_multiply(strassen_matrix *A,
                                            strassen_matrix *B) {
  uint64_t n = A->size;
  strassen_matrix *C = matrix_to_strass(matrix_ctor(n, n));
  if (n == 1) {
    C->ptr[0][0] = A->ptr[A->rmin][A->cmin] * B->ptr[B->rmin][B->cmin];
  } else {
    ...
    copy_matrix(subtract_matrix(subtract_matrix(add_matrix(P5, P1), P3), P7),
                C22);
  }
  return C;
}
...
uint64_t inversion_sub(int64_array *a, uint64_t lo, uint64_t pivot,
                       uint64_t hi) {
  uint64_t n1 = pivot - lo + 1;
  uint64_t n2 = hi - pivot;
  int64_t *L = (int64_t *)malloc(sizeof(*L) * (n1 + 1));
...
// returns the number of inversions required to permute array in non-decreasing
// order and sorts the input im not a fan that this function also sorts but the
// book wants me to i guess
