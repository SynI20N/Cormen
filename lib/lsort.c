...
int64_array* SORTCALL merge(int64_array* a, int64_array* b){
	int64_array* res = malloc(sizeof(*res));
	uint64_t pa = 0;
	uint64_t pb = 0;
	int64_t* A = a->ptr;
	int64_t* B = b->ptr;
	...
	uint64_t nb = b->count;
	uint64_t i = 0;
	int64_t* C = malloc(sizeof(int64_t) * (na + nb));
...
void SORTCALL merge_subroutine(int64_array* a, uint64_t lo, uint64_t pivot, uint64_t hi) {
	...
	int64_t* L = malloc(sizeof(*L) * (n1 + 1));
	int64_t* R = malloc(sizeof(*L) * (n2 + 1));
	uint64_t i, j;
	for(i = 0; i < n1; i++) {
		L[i] = a->ptr[lo + i];
	}
...
int64_t SORTCALL randomized_select(int64_t* a, int64_t lo, int64_t hi, uint64_t i) {
	if(lo == hi) {
		return a[lo];
	}
	int64_t q = hoare_partition(a, lo, hi);
	...
	if(i == k) {
		...
		return randomized_select(a, lo, q, i);
	}
	else return randomized_select(a, q + 1, hi, i - k);
}
...
SORTAPI int64_sub SORTCALL recursive_subarray(int64_t* a, int64_t lo, int64_t hi) {
	...
	/*if(hi - lo < 1499) {
		...
		int64_t* ptr = a + lo;
		int64_array* arr = malloc(sizeof(*arr));
		arr->ptr = ptr;
		arr->count = count;
		int64_sub sub = brute_force_subarray(arr);
		free(arr);
		...
		sub.low += lo;
		sub.high += lo;*/
		int64_sub sub = { .high = hi, .low = lo, .sum = a[lo] };
		return sub;
	}
	else {
...
void extend(int64_array* a, uint64_t k){
	a->ptr = realloc(a->ptr, sizeof(int64_t) * (a->count + k));
...
int64_array* SORTCALL add_bits(int64_array* i1, int64_array* i2) {
	...
	res->ptr = calloc(n, sizeof(int64_t));
	res->count = n;
	if(k > 0) {
		extend(i2, k);
	}
